...
class ThirdPartyEndpointTest {
    ...
    fun testValidRequestBody(vertx: Vertx, testContext: VertxTestContext)
    {
        ...
        vertx.createHttpServer()
                .requestHandler(garminRouter.getRouter())
                .listen(port,
                    testContext.succeeding { callEndpoint(vertx, testContext, thirdPartyPushService) }
                )
    }

    private fun callEndpoint(vertx: Vertx, tc: VertxTestContext, thirdPartyPushService: IThirdPartyPushService)
    {
                ...
                .subscribe(
                        { response ->
                            tc.verify {
                                ...
                                verify(thirdPartyPushService, times(2)).saveDataToOMH(any<ThirdPartyDailySummaryGarmin>())
                                tc.completeNow()
                            }
                        },
